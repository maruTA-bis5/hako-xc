...
class Log extends LogIO {
  ...
  function oilEnd($id, $name, $lName, $point) {
	...
	function ParkMoney($id, $name, $lName, $point, $str) {
	    global $init;
	    $this->out("{$init->tagName_}{$name}島{$point}{$init->_tagName}の<B>{$lName}</B>から、<B>{$str}</B>の収益が上がりました。",$id);
	}

	// 遊園地のイベント
	function ParkEvent($id, $name, $lName, $point, $str) {
	    global $init;
	    $this->out("{$init->tagName_}{$name}島{$point}{$init->_tagName}の<B>{$lName}</B>でイベントが開催され、<B>{$str}</B>の食料が消費されました。",$id);
	}
	
	// 遊園地のイベント増収
	function ParkEventLuck($id, $name, $lName, $point, $str) {
	    global $init;
	    $this->out("{$init->tagName_}{$name}島{$point}{$init->_tagName}の<B>{$lName}</B>で開催されたイベントが成功して<B>{$str}</B>の収益が上がりました。",$id);
	}

	// 遊園地のイベント減収
	function ParkEventLoss($id, $name, $lName, $point, $str) {
	    global $init;
	    $this->out("{$init->tagName_}{$name}島{$point}{$init->_tagName}の<B>{$lName}</B>で開催されたイベントが失敗して<B>{$str}</B>の損失がでました。",$id);
	}

	// 遊園地が閉園
	function ParkEnd($id, $name, $lName, $point) {
	    global $init;
	    $this->out("{$init->tagName_}{$name}島{$point}{$init->_tagName}の<B>{$lName}</B>は施設が老朽化したため閉園となりました。",$id);
	}
  ...
  function eruptionNormal($id, $name, $lName, $point) {
	...
	function NoSeaAround($id, $name, $comName, $point) {
		global $init;
		$this->out("{$init->tagName_}{$name}島{$init->_tagName}で予定されていた{$init->HtagName_}{$comName}{$init->_tagComName}は、予定地の{$init->tagName_}{$point}{$init->_tagName}の周辺に海がなかったため中止されました。",$id);
	}
	# 海がなくて失敗
	function NoSea($id, $name, $comName, $point) {
		global $init;
		$this->out("{$init->tagName_}{$name}島{$init->_tagName}で予定されていた{$init->HtagName_}{$comName}{$init->_tagComName}は、予定地が海でなかったため中止されました。",$id);
	}
	# 港がないので、造船失敗
	function NoPort($id, $name, $comName, $point) {
		global $init;
		$this->out("{$init->tagName_}{$name}島{$init->_tagName}で予定されていた{$init->HtagName_}{$comName}{$init->_tagComName}は、<b>港</b>がなかったため中止されました。",$id);
	}
	# 港閉鎖
	function ClosedPort($id, $name, $lName, $point) {
		global $init;
		$this->out("{$init->tagName_}{$name}島{$point}{$init->_tagName}の<B>{$lName}</B>は閉鎖したようです。",$id);
	}
	# 海賊船現る
	function VikingCome($id, $name, $point) {
		global $init;
		$this->out("{$init->tagName_}{$name}島{$point}{$init->_tagName}に<B>海賊船</B>出現！！",$id);
	}
	# 海賊船去る
	function VikingAway($id, $name, $point) {
		global $init;
		$this->out("{$init->tagName_}{$name}島{$point}{$init->_tagName}から<B>海賊船</B>がどこかに去っていきました。",$id);
	}
	# 海賊攻撃
	function VikingAttack($id, $name, $lName, $point) {
		global $init;
		$this->out("{$init->tagName_}{$name}島{$point}{$init->_tagName}の<b>{$lName}</b>は<B>海賊船</B>によって沈没させられました。",$id);
	}

	# 海賊船、強奪
	function RobViking($id, $name, $money, $food) {
		global $init;
		$this->out("{$init->tagName_}{$name}島{$init->_tagName}で<B>海賊</B>が<b>{$money}{$init->unitMoney}</b>の金と<b>{$food}{$init->unitFood}</b>の食料を強奪していきました。",$id);
	}
	# 船座礁
	function RunAground($id, $name, $lName, $point) {
		global $init;
		$this->out("{$init->tagName_}{$name}島{$point}{$init->_tagName}の<B>$lName</B>は{$init->tagDisaster_}座礁{$init->_tagDisaster}しました。",$id);
	}
}
